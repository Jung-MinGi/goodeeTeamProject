...
import com.gdj59.bookmall.beans.BuyVO;
...
public interface userMapper {
	...
	@Select("select * from payment_table where pm_id=#{user_id}")
	public List<BuyVO> 구매내역(String user_id);
	
	@Update("update cart set ordercnt=#{ordercnt}-1 where b_name=#{b_name}")
	public void modifyCart(CartVO cartVO);

	@Select("select ordercnt,b_price from cart where b_name=#{b_name} and user_id=#{user_id}")
